...
import net.minecraft.server.v1_8_R2.BlockLogAbstract;
...
public class WorldGenBigTree_v1_8_R2 extends WorldGenTreeAbstract {
    ...
    int h;
    int i;
    List<WorldGenBigTree_v1_8_R2.Position> j;
    boolean bigTreeFix;

    public WorldGenBigTree_v1_8_R2(boolean flag, boolean treeFix) {
        super(flag);
        bigTreeFix = treeFix;
    ...
    void a() {
        ...
        int i = (int) (1.382D + Math.pow(this.f * this.a / 13.0D, 2.0D));
        ...
        this.j.add(new WorldGenBigTree_v1_8_R2.Position(this.m.up(k), j));
        ...
        for (; k >= 0; --k) {
            ...
            if (f >= 0.0F) {
                for (int l = 0; l < i; ++l) {
                    double d0 = this.e * f * (this.k.nextFloat() + 0.328D);
                    double d1 = this.k.nextFloat() * 2.0F * 3.141592653589793D;
                    double d2 = d0 * Math.sin(d1) + 0.5D;
                    double d3 = d0 * Math.cos(d1) + 0.5D;
                    BlockPosition blockposition = this.m.a(d2, k - 1, d3);
                    ...
                    if (this.a(blockposition, blockposition1) == -1) {
                        ...
                        double d4 = blockposition.getY() - Math.sqrt(i1 * i1 + j1 * j1) * this.d;
                        ...
                        if (this.a(blockposition2, blockposition) == -1) {
                            this.j.add(new WorldGenBigTree_v1_8_R2.Position(blockposition, blockposition2.getY()));
                        }
                    }
                }
            }
    ...
    void a(BlockPosition blockposition, float f, IBlockData iblockdata) {
        ...
        for (int j = -i; j <= i; ++j) {
            for (int k = -i; k <= i; ++k) {
                if (Math.pow(Math.abs(j) + 0.5D, 2.0D) + Math.pow(Math.abs(k) + 0.5D, 2.0D) <= f * f) {
    ...
    void a(BlockPosition blockposition, BlockPosition blockposition1, Block block) {
        ...
        for (int j = 0; j <= i; ++j) {
            BlockPosition blockposition3 = blockposition.a(0.5F + j * f, 0.5F + j * f1, 0.5F + j * f2);
    ...
    void b() {
        ...
        while (iterator.hasNext()) {
            WorldGenBigTree_v1_8_R2.Position worldgenbigtree_position = (WorldGenBigTree_v1_8_R2.Position) iterator.next();

            this.a(worldgenbigtree_position);
        }

    }

    boolean c(int i) {
        return i >= this.a * 0.2D;
    }
    ...
    void d() {
        ...
        while (iterator.hasNext()) {
            ...
            if (!blockposition.equals(worldgenbigtree_position) && this.c(i - this.m.getY())) {
                this.a(blockposition, worldgenbigtree_position, Blocks.LOG);
            }
        }
